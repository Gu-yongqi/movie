<template>
  <div class="regin">
    <van-nav-bar title="注册" fixed placeholder z-index="999" left-arrow @click-left="$router.back()" />
    <div class="logo">
    ...
    <van-button round block type="info" native-type="submit" class="submit" @click="submit">注册</van-button>
    ...
    <div class="reg">
      <div @click="$router.push('/login')">已有账号</div>
      <div @click="$router.back()">暂不注册</div>
    </div>
  </div>
</template>
  ...
  methods: {
    submit() {
      ...
      this.axios({
        ...
        console.log(res);
        if (res.data.code == 0) {
          let { token, tokenExpired, uid } = res.data
          this.$toast('注册成功');
